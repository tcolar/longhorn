...
module bolts() {
  ...
  bolt(5*sepx-4.9,reary-4.2);
 
  //bolt(1.5*sepx,fronty+4.3);
  bolt(2*sepx,reary-4.2);
  bolt(5*sepx-4.9,-7);
...
//   // to add thickness
  ...
  // take this stuff away from the rest
    union() {
      ...
      translate([4.75*sepx-20,17.2+58+1,-3.5+1.75-0.5]) cube([11,5,11],true); // trrs jack
    }
...
module thumbcluster() {
    ...
    //kd=18;
    // position cluster
    translate([m*sepx,fronty-7.1*unit,baseline-0.75])
    union() {
        // rotate it to extrude along z plane
        difference() {
            rotate(90)
            rotate_extrude(angle=-90)
            {
                // front wall
                translate([7.07*unit,0,0]) square([0.5*unit,1.3*unit],false);
                // back wall
                translate([11.13*unit,0,0]) square([0.5*unit,2.4*unit], false);
                // top wall
                rotate(15)    
                    translate([7.16*unit,-4]) square([4.7*unit,0.25*unit], false);
            };
            // thumb holes        
            translate([2.4*unit,8.9*unit,10]) rotate(-15) cube([kw,kd,10],true);
            translate([6.5*unit,6.5*unit,10]) rotate(-45) cube([kw,kd,10],true);
            translate([8.7*unit,2.5*unit,10]) rotate(-75) cube([kw,kd,10],true);
        }
        // endcap
        rotate([90]) linear_extrude(height=0.5)  polygon(points=[[7.07*unit,0], [7.07*unit,1.3*unit], [11.63*unit,2.55*unit], [11.63*unit,0]]);
    }
}
...
union() {
  ...
  jack_caddy(); // jack caddy
  //jack();
  //thumbcluster();
}
// hole to thethumcluster for wiring
translate([4.5*sepx, fronty+7, baseline]) cube([sepx,10,6], false);
}
